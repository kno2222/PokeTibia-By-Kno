function onCastSpell(cid, var)
   ...
   local function setGender(cid, gender)
         if not isCreature(cid) then return true end    --alterado v1.5
   ...
   ["Yanma"] = "YanmaTeam",
    ...
    local level = getWildPokemonLevel(cid)
    ---------
    doDisapear(cid)
    ...
    addEvent(setGender, 150, cid, gender)         --alterado v1.5
    setCreatureMaxHealth(pk, maxLife)
    doCreatureAddHealth(pk, life-maxLife)
    setWildPokemonLevel(pk, level)
    ---------
