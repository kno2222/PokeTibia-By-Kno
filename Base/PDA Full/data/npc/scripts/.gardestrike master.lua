...
local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
        ...
        if (msgcontains(msg, 'clan') or msgcontains(msg, 'Clan')) then   --alterado v2.7
           if getPlayerClanName(cid) == 'Gardestrike' then       --alterado v2.7
              selfSay("You already are of the clan Gardestrike!", cid)
              talkState[talkUser] = 0
              return true
           elseif getPlayerClanNum(cid) ~= -1 then            --alterado v2.7
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 2 then
           ...
           setPlayerClan(cid, 'Gardestrike')  --alterado v2.7
           talkState[talkUser] = 0
           
        elseif (msgcontains(msg, 'leave') or msgcontains(msg, 'Leave')) then  --alterado v2.7  \/
        ...
        elseif (msgcontains(msg, 'shiny') or msgcontains(msg, 'Shiny')) then    --alterado v2.7
           ...
           if getPlayerClanName(cid) ~= 'Gardestrike' and ball == "Hitmontop" then  --alterado v2.7
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 4 then
	       ...
	       if not isInArray({"Hitmonlee", "Hitmonchan", "Hitmontop"}, getItemAttribute(pb, "poke")) then     --alterado v2.4
	          selfSay("Put a Hitmonchan's pokeball or a Hitmonlee's pokeball or a Hitmontop's pokeball atleast +20 in main slot!", cid)
              talkState[talkUser] = 0
              return true
           end
           
           if getPlayerClanName(cid) ~= 'Gardestrike' and getItemAttribute(pb, "poke") == "Hitmontop" then --alterado v2.7
              selfSay("You aren't of the clan Gardestrike! Get out of here!", cid)   --alterado v2.4
              talkState[talkUser] = 0
              return true 
           end  
           ...
           if doPlayerRemoveMoney(cid, 20000000) == true then
		      ...
		      ----------------------
		      local pokeinfo = getPokemonStatus("Shiny "..name)
	          ...
	          local leveltable = getPokemonExperienceTable("Shiny "..name)
		      ...
		      doItemSetAttribute(pb, "fakedesc", "Contains a Shiny "..name..".")
	          -----------------------
		      if useKpdoDlls then
