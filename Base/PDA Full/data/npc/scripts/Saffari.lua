...
if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 3 then
   selfSay('Hello, i can let you enter in the saffari zone for 500 Dollars, do you accept?')--alterado v2.9
   focus = cid
   talk_start = os.clock()
elseif (msgcontains(msg, 'yes') and focus == cid ) then  --alterado v2.9
   if getPlayerItemCount(cid,2391) >= 1 or getPlayerItemCount(cid,2394) >= 1 or getPlayerItemCount(cid,2392) >= 1 or getPlayerItemCount(cid,2393) >= 1 or getPlayerItemCount(cid, 12617) >= 1 then
      selfSay("You can't enter in the saffari zone with any type of pokeballs! Cya")                                                                   --alterado v2.9 /\
      focus = 0
      talk_start = 0
   elseif getPlayerStorageValue(cid, 98796) >= 1 or getPlayerStorageValue(cid, 98797) >= 1 then
      ...
      focus = 0
      talk_start = 0
   elseif doPlayerRemoveMoney(cid, 50000) then --500dl --alterado v2.9
      setPlayerStorageValue(cid, 98796, 1)
      setPlayerStorageValue(cid, 98797, 1)
      doPlayerAddItem(cid, 12617, 30)  --alterado v2.9
      doTeleportThing(cid, SafariEnter)
      doSendMagicEffect(getThingPos(cid), 21)
      talk_start = os.clock()
   else
      selfSay("You don't have that much of money. Cya")   --alterado v2.9
      focus = 0
      talk_start = 0
   end
