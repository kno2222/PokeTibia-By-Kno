...
if getPlayerStorageValue(mypoke, 212123) >= 1 then
   ...
   cdzin = "move"..it..""       --alterado v2.5
	...
	if getLevel(mypoke) < move.level then
		doPlayerSendTextMessage(cid, 26, "Your pokemon doesn't have enough level to use this move.")
	return 0
	end

	if getCD(getPlayerSlotItem(cid, 8).uid, cdzin) > 0 and getCD(getPlayerSlotItem(cid, 8).uid, cdzin) < (move.cd + 2) then
		 doPlayerSendCancel(cid, "[MOVE] You have to wait ["..getCD(getPlayerSlotItem(cid, 8).uid, cdzin).."] seconds to use "..move.name.." again.")
...
if move.target == 1 then
	...
	doPlayerSendTextMessage(cid, 25, "You don\'t have any targets.")
	return 0
	end
